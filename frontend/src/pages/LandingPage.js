...
const LandingPage = () => {
  return (
    <div className="landing-page min-h-screen bg-primary-50 text-slate-950">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 text-sm font-medium text-slate-700 md:flex">
          <a href="#platform" className="transition hover:text-primary-700">Platform</a>
          <a href="#workflow" className="transition hover:text-primary-700">Workflow</a>
          <a href="#features" className="transition hover:text-primary-700">Features</a>
        </nav>

        <Link
          to="/login"
          className="inline-flex items-center gap-2 rounded-md bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-800"
        >
      ...
      <main>
        <section className="mx-auto grid max-w-7xl items-center gap-10 px-4 pb-10 pt-6 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:pb-14">
          <div className="max-w-2xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-md border border-primary-200 bg-white px-3 py-2 text-sm font-semibold text-primary-800 shadow-sm">
              <FaShieldAlt />
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/login"
                className="inline-flex items-center gap-2 rounded-md bg-primary-700 px-5 py-3 font-semibold text-white shadow-sm transition hover:bg-primary-800"
              >
                Start managing <FaArrowRight />
              </Link>
              <a
                href="#features"
                className="inline-flex items-center rounded-md border border-slate-300 bg-white px-5 py-3 font-semibold text-slate-900 shadow-sm transition hover:border-primary-300 hover:text-primary-800"
              >
                View features
              </a>
            </div>
          ...
          <div className="relative min-h-[520px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.16)]">
            <div className="absolute inset-x-0 top-0 h-48 bg-primary-800" />
            <div className="relative grid h-full min-h-[520px] grid-rows-[auto_1fr] p-5 sm:p-6">
              <div className="flex items-center justify-between rounded-md bg-white/95 p-4 shadow-sm">
                ...
                <div className="rounded-md bg-primary-100 p-3 text-primary-700">
                  <FaStore />
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                ...
                <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-md bg-primary-100 px-3 py-1 text-sm font-bold text-primary-800">+12.5%</span>
                  </div>
                  <div className="mt-7 flex h-40 items-end gap-3">
                    {[42, 64, 52, 78, 68, 91, 84].map((height, index) => (
                      <div key={index} className="flex flex-1 items-end rounded-t-md bg-primary-100">
                        <div
                          className="w-full rounded-t-md bg-primary-700"
                          style={{ height: `${height}%` }}
                        />
                      </div>
                    ))}
                  ...
                  <div className="mt-6 space-y-3">
                    {operatingPoints.map((point) => (
                      <div key={point} className="flex items-start gap-3 text-sm text-slate-700">
                        <FaCheckCircle className="mt-1 shrink-0 text-primary-700" />
                        <span>{point}</span>
                      </div>
                    ))}
                  </div>
        ...
        <section id="workflow" className="mx-auto grid max-w-7xl gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
          <div>
            <p className="text-sm font-bold uppercase text-primary-800">Daily workflow</p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={title} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex h-10 w-10 items-center justify-center rounded-md bg-primary-100 text-primary-800">
                  <Icon />
                </div>
        ...
        <section id="features" className="bg-slate-950 py-16 text-white">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl">
              <p className="text-sm font-bold uppercase text-primary-300">Core features</p>
              ...
              {features.map((feature) => {
                ...
                return (
                  <div key={feature.title} className="rounded-lg border border-white/10 bg-white/[0.06] p-6">
                    <div className="flex h-11 w-11 items-center justify-center rounded-md bg-white text-primary-800">
                      <Icon />
                    </div>
